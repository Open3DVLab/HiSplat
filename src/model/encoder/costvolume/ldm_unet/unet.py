    ...
    def __init__(
        ...
        if self.ffn_after_cross_attn:
            ...
            self.ffn = nn.Sequential(
                nn.Conv3d(channels, channels * 4, 1, 1, 0),
    ...
    def __init__(
        ...
        if postnorm:
            self.out = nn.Sequential(
                ...
                else zero_module(normalization(out_channels, channels_per_group=channels_per_group)),
                nn.SiLU(),
            )
        else:
            self.out = nn.Sequential(
...
class StackUNet(nn.Module):
    def __init__(
        ...
        for i in range(num_stacks - 1):
            self.convs.append(zero_module(conv_nd(dims, out_channels, in_channels, 3, padding=1)))
