...
from ....geometry.projection import get_world_rays_and_cam_xy_sin
...
class Gaussians:
    ...
    # normal: Float[Tensor, "*batch 3"] | None
    # distance: Float[Tensor, "*batch 3"] | None
...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        decouple_scales = scales * depths[..., None].detach() * multiplier[..., None]
        scales = scales * depths[..., None] * multiplier[..., None]  # TODO: why? the far points need big scale?
        ...
        origins, directions, xy_sin = get_world_rays_and_cam_xy_sin(coordinates, extrinsics, intrinsics)
        # We fix it and use the correct distance
        # TODO: whether to use xy_sin
        if get_cfg()["use_xy_sin"]:
            means = origins + directions * depths[..., None] / (xy_sin + 1e-8)
        else:
