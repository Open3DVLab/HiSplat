...
from .dino.layers.attention import (
    ...
    def __init__(self, args):
        ...
        for _ in range(self.dino_cfg["cross_interval_layers"] - 1):
            self.self_attn_blocks.append(
                CrossBlock(
                    dim=self.decoder_cfg["d_model"],
                    num_heads=self.decoder_cfg["nhead"],
                    attn_class=self_attn_class,
                    ffn_layer=ffn_class,
                    **self.decoder_cfg,
                )
            )
        ...
        self.proj = nn.Sequential(nn.Conv2d(vit_ch, ch * 4, 3, stride=1, padding=1), nn.BatchNorm2d(ch * 4), nn.SiLU())
        self.upsampler0 = nn.Sequential(
            nn.ConvTranspose2d(ch * 4, ch * 2, 4, stride=2, padding=1), nn.BatchNorm2d(ch * 2), nn.SiLU()
        )
        self.upsampler1 = nn.Sequential(
            nn.ConvTranspose2d(ch * 2, ch, 4, stride=2, padding=1), nn.BatchNorm2d(ch), nn.SiLU()
        )
...
class CostRegNet2D(nn.Module):
    def __init__(self, in_channels, base_channel=8):
        ...
        self.conv3 = Conv3d(
            base_channel * 2, base_channel * 4, kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)
        )
        self.conv4 = Conv3d(base_channel * 4, base_channel * 4, padding=1)

        self.conv5 = Conv3d(
            base_channel * 4, base_channel * 8, kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if attention_type == "FLASH2":
            ...
            self.attn = CrossLinearAttention(
                dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
                proj_bias=proj_bias,
    ...
    with torch.no_grad():
        ...
        if n % 2 == 1:
            prob_volume_sum4 = n * F.avg_pool3d(
                F.pad(p.unsqueeze(1), pad=[0, 0, 0, 0, n // 2, n // 2]), (n, 1, 1), stride=1, padding=0
            ).squeeze(1)
        else:
            prob_volume_sum4 = n * F.avg_pool3d(
                F.pad(p.unsqueeze(1), pad=[0, 0, 0, 0, n // 2 - 1, n // 2]), (n, 1, 1), stride=1, padding=0
            ).squeeze(1)
...
def init_range(cur_depth, ndepths, device, dtype, H, W):
    if len(cur_depth.shape) == 2:
        ...
        depth_range_samples = cur_depth_min.unsqueeze(1) + (
            torch.arange(0, ndepths, device=device, dtype=dtype, requires_grad=False).reshape(1, -1)
        ...
        depth_range_samples = cur_depth_min.unsqueeze(1) + (
            torch.arange(0, ndepths, device=device, dtype=dtype, requires_grad=False).reshape(1, -1, 1, 1)
...
def init_inverse_range(cur_depth, ndepths, device, dtype, H, W):
    if len(cur_depth.shape) == 2:
        ...
        itv = torch.arange(0, ndepths, device=device, dtype=dtype, requires_grad=False).reshape(1, -1, 1, 1).repeat(
            1, 1, H, W
        ) / (ndepths - 1)  # 1 D H W
        inverse_depth_hypo = (
            inverse_depth_max[:, None, None, None] + (inverse_depth_min - inverse_depth_max)[:, None, None, None] * itv
        )
    else:
        inverse_depth_min = 1.0 / cur_depth[..., 0]  # (B,H,W)
        inverse_depth_max = 1.0 / cur_depth[..., -1]
        itv = torch.arange(0, ndepths, device=device, dtype=dtype, requires_grad=False).reshape(1, -1, 1, 1).repeat(
            1, 1, H, W
        ) / (ndepths - 1)  # 1 D H W
        inverse_depth_hypo = (
            inverse_depth_max[:, None, :, :] + (inverse_depth_min - inverse_depth_max)[:, None, :, :] * itv
        )
...
def schedule_inverse_range(depth, depth_hypo, ndepths, split_itv, H, W, shift=False):
    # We should not make the depth beyond the near and far
    last_depth_itv = 1.0 / depth_hypo[:, 2, :, :] - 1.0 / depth_hypo[:, 1, :, :]
    max_inverse, min_inverse = 1 / get_cfg()["dataset"]["near"], 1 / get_cfg()["dataset"]["far"]
    inverse_min_depth = (1 / depth + split_itv * last_depth_itv).clamp(min_inverse, max_inverse)  # B H W
    inverse_max_depth = (1 / depth - split_itv * last_depth_itv).clamp(min_inverse, max_inverse)  # B H W
    ...
    # cur_depth_max: (B, H, W)
    itv = torch.arange(
        0, ndepths, device=inverse_min_depth.device, dtype=inverse_min_depth.dtype, requires_grad=False
    ).reshape(1, -1, 1, 1).repeat(1, 1, H // 2, W // 2) / (ndepths - 1)  # 1 D H W

    inverse_depth_hypo = (
        inverse_max_depth[:, None, :, :] + (inverse_min_depth - inverse_max_depth)[:, None, :, :] * itv
    )  # B D H W
    inverse_depth_hypo = F.interpolate(
        inverse_depth_hypo.unsqueeze(1), [ndepths, H, W], mode="trilinear", align_corners=True
    ).squeeze(1)
    return 1.0 / inverse_depth_hypo
...
def schedule_range(cur_depth, ndepth, depth_inteval_pixel, H, W):
    ...
    depth_range_samples = cur_depth_min.unsqueeze(1) + (
        torch.arange(0, ndepth, device=cur_depth.device, dtype=cur_depth.dtype, requires_grad=False).reshape(
            1, -1, 1, 1
        )
        * new_interval.unsqueeze(1)
    )
    depth_range_samples = F.interpolate(
        depth_range_samples.unsqueeze(1), [ndepth, H, W], mode="trilinear", align_corners=True
    ).squeeze(1)
    return depth_range_samples
