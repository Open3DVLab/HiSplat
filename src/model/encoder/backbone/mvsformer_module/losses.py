...
def get_multi_stage_losses(loss_args, depth_types, outputs, depth_gt_ms, mask_ms, depth_interval, inverse_depth):
    ...
    for stage_inputs, stage_key in [(outputs[k], k) for k in stage_keys]:
        ...
        if depth_type == "ce":
            ...
            gt_index_volume = (
                (depth_values_right <= depth_gt_volume).to(torch.float32).sum(dim=1, keepdims=True).to(torch.long)
            )
...
def get_loss(loss_arg, depth_type, outputs, depth_gt_ms_tmp, mask_ms_tmp, depth_interval, inverse_depth):
    if depth_type == "re":
        loss_dict = reg_loss(outputs, depth_gt_ms_tmp, mask_ms_tmp, dlossw=[1, 1, 1, 1], depth_interval=depth_interval)
    elif depth_type == "ce":
        loss_dict = ce_loss(
            outputs,
            depth_gt_ms_tmp,
            mask_ms_tmp,
            dlossw=[1, 1, 1, 1],
            focal=loss_arg["focal"],
            gamma=loss_arg["gamma"],
