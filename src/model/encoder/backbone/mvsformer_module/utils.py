    ...
    def lr_lambda(current_step):
        if current_step < num_warmup_steps:
            ...
            lr_weight = min_lr + (1.0 - min_lr) * 0.5 * (
                1.0 + math.cos(math.pi * (current_step - num_warmup_steps) / (total_steps - num_warmup_steps))
            )
        return lr_weight
...
def get_parameter_groups(opt_args, model, freeze_vit=None):
    ...
    param_groups.append(
        {
            "params": [p for n, p in model.named_parameters() if p.requires_grad and not n.startswith("vit.")],
            "lr": opt_args["lr"],
            "weight_decay": 0.0,
        }
    )
    ...
    if freeze_vit is False:
        ...
        if lora_cfg is not None and lora_cfg["rank"] > 0:  # use lora
            param_groups.append(
                {
                    "params": [
                        ...
                        if p.requires_grad and n.startswith("vit.") and "lora_" in n
                    ],
                    "lr": opt_args["vit_lr"],
            ...
            param_groups.append(
                {
                    "params": [p for n, p in model.named_parameters() if p.requires_grad and n.startswith("vit.")],
                    "lr": opt_args["vit_lr"],
                    "weight_decay": opt_args["weight_decay"],
                }
            )
...
def init_model(config):
    if "DINOv2" in config["arch"]["args"]["model_type"]:
        from networks.DINOv2_mvsformer_model import DINOv2MVSNet

        model = DINOv2MVSNet(config["arch"]["args"])
    elif config["arch"]["args"]["model_type"] == "casmvs":
        from networks.casmvs_model import CasMVSNet
