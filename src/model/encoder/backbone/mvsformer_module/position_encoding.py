...
class EpipoleEncodingSineNorm(nn.Module):
    ...
    def forward(self, feat, Fmat):
        ...
        y, x = torch.meshgrid(
            [
                torch.arange(0, height, dtype=torch.float32, device=feat.device),
                torch.arange(0, width, dtype=torch.float32, device=feat.device),
            ],
            indexing="ij",
        )
...
def get_position_3d(
    B, H, W, K, depth_values, depth_min, depth_max, height_min, height_max, width_min, width_max, normalize=True
):
    num_depth = depth_values.shape[1]
    with torch.no_grad():
        y, x = torch.meshgrid(
            [
                torch.arange(0, H, dtype=torch.float32, device=K.device),
                torch.arange(0, W, dtype=torch.float32, device=K.device),
            ],
            indexing="ij",
        )
        y, x = y.contiguous(), x.contiguous()
        ...
        position3d = xyz.unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_values.view(
            B, 1, num_depth, -1
        )  # [B, 3, 1, H*W]->[B,3,D,H*W]
        if normalize:  # minmax normalization
            ...
            position3d[:, 2] = (torch.clamp(position3d[:, 2], depth_min, depth_max) - depth_min) / (
