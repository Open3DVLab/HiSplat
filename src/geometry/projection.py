...
def get_world_rays_and_cam_xy_sin(
    coordinates: Float[Tensor, "*#batch dim"],
    extrinsics: Float[Tensor, "*#batch dim+2 dim+2"],
    intrinsics: Float[Tensor, "*#batch dim+1 dim+1"],
):
    """Get world-space ray directions, origins and camera-space sine theta to xy plane"""
    # Get camera-space ray directions.
    directions = unproject(
        coordinates,
        torch.ones_like(coordinates[..., 0]),
    ...
    # Get xy_sin in camera-space.
    xy_sin = directions[..., -1:].abs()
    ...
    return origins, directions, xy_sin
