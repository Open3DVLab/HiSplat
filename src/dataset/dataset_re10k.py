...
class DatasetRE10kCfg(DatasetCfgCommon):
    name: Literal["re10k", "dtu", "replica"]
    roots: list[Path]
...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        self.test_times_per_scene = get_cfg()["dataset"]["test_times_per_scene"]
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            times_per_scene = self.test_times_per_scene
    ...
    def __len__(self) -> int:
        return (
            min(len(self.index.keys()) * self.test_times_per_scene, self.cfg.test_len)
            if self.stage == "test" and self.cfg.test_len > 0
            else len(self.index.keys()) * self.test_times_per_scene
        )
