...
def get_example_keys(stage: Literal["test", "train"]) -> list[str]:
    ...
    key_id_test = [1, 8, 21, 30, 31, 34, 38, 40, 41, 45, 55, 63, 82, 103, 110, 114]
    key_id_train = list(set(list(range(1, 129))) - set(key_id_test) - {25, 26, 27, 54, 73, 78, 79, 80, 81})
    if stage == "test":
        keys = [f"scan{k}_train" for k in key_id_test]
    elif stage == "train":
        keys = [f"scan{k}_train" for k in key_id_train]
    else:
        raise NotImplementedError(f"This stage: {stage} is not Implemented!")
